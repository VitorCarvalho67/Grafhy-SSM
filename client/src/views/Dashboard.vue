...
export default {
    ...
    methods: {
        ...
        async fetchSolicitations() {
            try {
                const response = await axios.get(`http://localhost:9999/solicitation/${this.user}`);
                const solicitations = response.data;
                for (let solicitation of solicitations) {
                    const userResponse = await axios.get(`http://localhost:9999/users/id/${solicitation.id_users1}`);
        ...
        async sendSolicitation() {
            ...
            if (this.user && otherUserId) {
                try {
                    const response = await axios.post(`http://localhost:9999/solicitation/`, {
                        id_users1: this.user,
        ...
        async respondToSolicitation(user1, resposta) {
            ...
            try {
                const solicitationResponse = await axios.post(`http://localhost:9999/solicitation/accept`, {
                    id_users1: user1,
                    id_users2: this.user,
                    response: resposta
                });
        ...
        async fetchContacts() {
            try {
                const response = await axios.get(`http://localhost:9999/contacts/${this.user}`);
                const contacts = response.data;
                for (let contact of contacts) {
                    const userResponse = await axios.get(`http://localhost:9999/users/id/${contact.id_users1}`);
